...
- When a clip contains both a concrete number/stat AND a surprising human, relationship, or life outcome, lead with the surprising outcome as the hook. A number is a supporting detail, not the default hook — don't reach for the dollar figure just because it's concrete.
- Ask: what's the single most surprising or "wait, what?" moment for a viewer? Hook on that. (e.g. "they moved in together 3 weeks after meeting" beats "it started with a $2k paycheck".)
...
TITLE RULES:
- If the clip has both a number and a surprising human/relationship outcome,
  the title should lead with the surprising outcome, not the number. Example:
  prefer "They Moved In Together 3 Weeks After Meeting" over "Brez Paid $2K
  Upfront to Start a Community."
...
- Prioritize a smooth, natural flow over cramming in every fact. It's fine to compress or drop a minor detail (e.g. summarize a back-and-forth as "no negotiation") if it makes the recap read more naturally. Still stay grounded in the transcript and never invent.
